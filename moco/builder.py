...
class MoCo(nn.Module):
    ...
    def __init__(self, dim=300, K=65536, m=0.999, T=0.07, mlp=False):
        ...
        # create the encoders
        self.encoder_q = AutoModelForSequenceClassification.from_pretrained(
            pretrained_model_name_or_path='nghuyong/ernie-2.0-large-en',
            config=config
        )
        self.encoder_k = AutoModelForSequenceClassification.from_pretrained(
            pretrained_model_name_or_path='nghuyong/ernie-2.0-large-en',
            config=config)
    ...
    def forward(self, sen_q, sen_k, mask_q, mask_k):
        """
        Input:
        ...
        q = self.encoder_q(sen_q, mask_q)  # queries: NxC
